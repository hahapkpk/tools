...
- JPEG 质量 0.92
- 文件名自动改为 `.jpg`
- 长边超过 8192px（如手机全景图）自动等比缩小，避免超出浏览器画布上限
- 单张转换失败只跳过该张，并在提示里列出原因，其余图片继续上传
...
2. 每隔 2～5 秒（递增退避）轮询 `zones/list` API 检查 token 变化
3. Token 变化 = 服务器已处理 → 自动 hash 路由切换刷新图库
4. 最多等 12 次，超时改为软刷新；只有软刷新无法重挂载图库时才整页重载
5. 等待期间点击悬浮球可立即刷新
...
- 每次挂载都按目标幂等注册（`__iCloudUploaderPasteHandler` 标记），不会因首次挂载时 `<body>` 未就绪而永久失效
...
| 解码/编码卡住 | 20s / 30s 超时后按失败处理，面板不会卡在忙碌态 |
| `input.files` 赋值被浏览器拒绝 | 提示改用“选择图片”手动选择 |
| 找不到文件输入框 | 回退到拖拽通道，并注明“未经验证” |
| 无法软刷新图库 | 才整页重载（会丢失页面状态） |
