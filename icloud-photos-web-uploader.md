...
1. 向 iCloud 交接文件前记录 CloudKit sync token，避免错过快速变化
2. 文件交接后每隔 2～5 秒轮询 `zones/list` API
3. Token 变化 = 服务器已处理 → 自动 hash 路由切换刷新图库
4. 轮询以总计 30 秒为截止时间，不会因退避累计到约 54 秒
5. 超时或无法读取 token 时只尝试软刷新，不再强制整页重载
6. 新上传批次用代际令牌取消旧轮询；等待期间点击悬浮球可立即软刷新
...
| 无法软刷新图库 | 提示手动切换视图，不整页重载，不丢失页面状态 |
